...
local function MergeOldPositions()	-- TODO delete after while
	if TKUIOptionsGlobal[T.realm][T.name] then
		if not TKUIPositionsPerChar then
			TKUIPositionsPerChar = TKUIPositions
		end
		if not TKUIPositionsPerChar.merged then
			local backup = TKUIPositions
			TKUIPositionsPerChar = {}
			TKUIPositionsPerChar["1"] = backup
			TKUIPositionsPerChar.merged = true
			TKUIPositionsPerChar["1"]["1"] = nil -- T.CurrentProfile calls early and create empty table so remove this
		end
	else
		if not TKUIPositions.merged then
			local backup = TKUIPositions
			TKUIPositions = {}
			TKUIPositions["1"] = backup
			TKUIPositions.merged = true
			TKUIPositions["1"]["1"] = nil
		end
	end
end
...
controls.x:SetFont(C.font.editmode_font, C.font.editmode_size, C.font.editmode_style)
controls.x:SetPoint("RIGHT", controls, "LEFT", -10, 0)

controls.y = controls:CreateFontString(nil, "OVERLAY")
controls.y:SetFont(C.font.editmode_font, C.font.editmode_size, C.font.editmode_style)
...
local CreateMover = function(frame, unit)
	...
	if unit then
		...
		mover.backdrop:SetBackdropBorderColor(1, 0.36, 0)
	else
		mover:SetTemplate("Transparent")
		mover:SetBackdropBorderColor(1, 0.36, 0)
	end
	...
	mover:SetScript("OnLeave", function(self)
		...
		self.backdrop:SetBackdropBorderColor(1, 0.36, 0)
	...
	mover.name:SetFont(C.font.editmode_font, C.font.editmode_size, C.font.editmode_style)
