...
local function OnEnter(element)
	...
	GameTooltip:SetOwner(element, "ANCHOR_BOTTOM", 0, -5)	-- TKUI
...
local function OnMouseUp(element, btn)
	if btn == "MiddleButton" then
		if element.outAlpha == 0 then
			element.outAlpha = 1
			TKUISettings.Reputation = true
		else
			element.outAlpha = 0
			TKUISettings.Reputation = false
		end
	else
...
local function CheckAlpha(element)
	if TKUISettings and TKUISettings.Reputation == true then
		element.outAlpha = 1
...
local function Update(self, event, unit)
	...
	if(name) then
		...
		if(element.colorStanding) then
			...
			element.bg:SetVertexColor(colors[1], colors[2], colors[3], 0.2)	-- TKUI
		end
