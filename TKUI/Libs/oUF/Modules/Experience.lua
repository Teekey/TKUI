...
local function OnMouseUp(element, btn)
	if btn == "MiddleButton" then
		if element.outAlpha == 0 then
			element.outAlpha = 1
			TKUISettings.Experience = true
		else
			element.outAlpha = 0
			TKUISettings.Experience = false
		end
	end
end

local function CheckAlpha(element)
	if TKUISettings and TKUISettings.Experience == true then
		element.outAlpha = 1
...
local function UpdateColor(element, isHonor, isRested)
	...
	element.bg:SetVertexColor(r, g, b, 0.2) -- TKUI
