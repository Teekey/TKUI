...
if not ObjectPoolMixin then
    ...
    function ObjectPoolMixin:Acquire()
        ...
        if numInactive > 0 then
            local obj = self.inactiveObjects[numInactive]
...
if not CreateFromMixins then
    function CreateFromMixins(...)
        local mixin = {}
        for i = 1, select("#", ...) do
            local mixin_n = select(i, ...)
            for k, v in pairs(mixin_n) do
                mixin[k] = v
            end
        end
        return mixin
...
if not CreateTexturePool then
    function CreateTexturePool(parent, layer, subLayer, templateType)
...
if not CreateFramePool then
    function CreateFramePool(frameType, parent, template, resetterFunc)
        ...
        pool:OnLoad(
            ...
            resetterFunc or function(_, frame)
                frame:Hide()
            end
        )
        return pool
