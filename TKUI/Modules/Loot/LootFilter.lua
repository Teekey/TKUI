...
local function SaveCustomFilters()
    ...
    wipe(TKUILootFilter)
    for itemID, value in pairs(T.LootFilterCustom) do
...
local function AddToCustomFilter(input)
    ...
    if itemName then
        T.LootFilterCustom[itemID] = true
...
local function RemoveFromCustomFilter(input)
    ...
    if T.LootFilterCustom[itemID] then
        local itemName, itemLink = GetItemInfo(itemID)
        T.LootFilterCustom[itemID] = nil
        SaveCustomFilters()  -- Save changes
...
local function ListCustomFilter()
    ...
    local count = 0
    for itemID in pairs(T.LootFilterCustom) do
        local itemName, itemLink = GetItemInfo(itemID)
...
local function ShouldLootItem()
    ...
    if T.LootFilterItems[itemID] or T.LootFilterCustom[itemID] then
    ...
    for i = ItemCount, 1, -1 do
        ...
        if Item["Link"] then
            ...
            if itemID then
                ...
                iconString = iconTexture and ("|T" .. iconTexture .. ":0|t ") or ""
            end
        end
        ...
        if not Locked then
            local itemID = GetItemIDFromLink(Item["Link"])
            if T.LootFilterItems[itemID] then
                print("|cFFFF5C00Filtered:|r " .. iconString .. (Item["Link"] or "Unknown Item") .. " (Ignored item)")
            elseif SlotType == 3 and T.LootFilterCurrency[itemID] then
                print("|cFFFF5C00Filtered:|r " ..
...
SlashCmdList["LOOTFILTER"] = function(msg)
    ...
    elseif command == "clear" then
        ...
        print("|cFFFF5C00TKUI LootFilter Commands:|r")
        print("|cFFFDAA48/lf add [itemID or item link]|r - Add an item to the custom filter")
        print("|cFFFDAA48/lf remove [itemID or item link]|r - Remove an item from the custom filter")
        print("|cFFFDAA48/lf list|r - List all items in the custom filter")
        print("|cFFFDAA48/lf clear|r - Clear all items from the custom filter")
    end
end
...
frame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        for itemID, value in pairs(TKUILootFilter) do
            T.LootFilterCustom[itemID] = value
        end
