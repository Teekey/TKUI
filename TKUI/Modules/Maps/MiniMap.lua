	...
	resetZoom:SetScript("OnEvent", function(self, event)
		if Minimap:GetZoom() > 0 then
			...
			elseif resetting == 0 then
				resetting = 1
				C_Timer.After(C.minimap.reset_time, function()
					Minimap:SetZoom(0)
					resetting = 0
				end)
			end
...
whoClickedText:SetFont(C.media.normal_font, 16, "OUTLINE")
...
local function OnMinimapPing(self, event, unit)
	...
	whoClickedText:SetTextColor(unpack(T.oUF_colors.class[class]))
...
zoneText:SetFont(C.media.normal_font, 16, "OUTLINE")
