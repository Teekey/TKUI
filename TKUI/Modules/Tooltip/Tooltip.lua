...
for _, tt in pairs(tooltips) do
	if not C_AddOns.IsAddOnLoaded("Aurora") then
		...
		tt.GetBackdropColor = function() return C.media.backdrop_color[1], C.media.backdrop_color[2],
				C.media.backdrop_color[3], C.media.backdrop_alpha end
...
GameTooltipStatusBar:SetScript("OnValueChanged", function(self, value)
	...
	if unit then
		...
		if not self.text then
			...
			self.text:SetFont(C.media.normal_font, 12, "OUTLINE")
			self.text:SetPoint("CENTER", GameTooltipStatusBar, 0, 0)
		end
		self.text:Show()
		local hp = T.ShortValue(min)
...
if C.tooltip.hidebuttons == true then
	local CombatHideActionButtonsTooltip = function(self)
		if not IsShiftKeyDown() and (InCombatLockdown() and C.tooltip.hide_combat) then
			self:Hide()
		end
	end
