...
local function ColorUnitName(name, guid)
    ...
    if guid then
        ...
        elseif unitType == "Creature" or unitType == "Vehicle" then
            ...
            local hostileColor = T.oUF_colors.reaction[2] -- Assuming 2 is the index for hostile
...
local function OutputMessage(message)
    ...
    if C.options.outputChat then
        ...
        local strippedMessage = fullMessage:gsub("|c%x%x%x%x%x%x%x%x", ""):gsub("|r", ""):gsub("|T.-|t", "")
...
local function HandleSpellCast(spellId, sourceName, sourceGUID, destName, destGUID, extraSpellId, extraSpellName, isInterrupt)
    ...
    local message
    if C.options.enableTaunt and T.spells.taunts[spellId] then
        message = sourceColor .. " taunted " .. destColor .. " with " .. spellLink .. "."
    elseif C.options.enableInterrupt and T.spells.interrupts[spellId] and isInterrupt then
        message = sourceColor .. " interrupted " .. destColor .. "'s " .. extraSpellLink .. " with " .. spellLink .. "."
    elseif C.options.enableDispel and T.spells.dispels[spellId] then
        message = sourceColor .. " dispelled " .. destColor .. "'s " .. extraSpellLink .. " with " .. spellLink .. "."
    elseif C.options.enableCrowdControl and T.spells.crowdControl[spellId] then
    ...
    if C.options.enableDeath then
        ...
        if lastDamage then
            ...
            elseif lastDamage.damageType ~= "" then
                ...
                deathMessage = string_format("%s died to %s %s (%d %s Damage).", destColor, sourceColor, spellLink, lastDamage.amount, GetSchoolString(lastDamage.school))
            end
        else
...
eventHandlers = {
    ...
    SPELL_AURA_BROKEN = function(sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, spellId)
        if ShouldProcessUnit(sourceFlags, destFlags, true) and T.spells.crowdControl[spellId] then
