...
local isToggling = false -- Flag to prevent re-entrance
local toggleDelay = 0.1 -- Delay in seconds

local function Stuffing_Toggle()
	if isToggling then return end -- Prevent re-entrance
	isToggling = true -- Set the flag

	C_Timer.After(toggleDelay, function() -- Use a timer to delay the toggle
		if Stuffing.frame:IsShown() then
			Stuffing.frame:Hide()
		else
			Stuffing.frame:Show()
		end
		isToggling = false -- Reset the flag after the toggle
	end)
end
...
function Stuffing:SlotUpdate(b)
	...
	-- Set all slot color to default TKUI on update
	...
	if b.frame.UpgradeIcon then
		b.frame.UpgradeIcon:SetPoint("TOPLEFT", 36/2.7, -36/2.7)
		b.frame.UpgradeIcon:SetSize(36/1.7, 36/1.7)
...
function Stuffing:CreateReagentContainer()
	...
	for i = 1, 98 do
		...
		elseif NumButtons == 10 then
			button:SetPoint("TOPRIGHT", LastRowButton, "TOPRIGHT", 0, -(6 + 36))
			button:SetPoint("BOTTOMLEFT", LastRowButton, "BOTTOMLEFT", 0, -(6 + 36))
			LastRowButton = button
			NumRows = NumRows + 1
			NumButtons = 1
		else
			button:SetPoint("TOPRIGHT", LastButton, "TOPRIGHT", (6 + 36), 0)
			button:SetPoint("BOTTOMLEFT", LastButton, "BOTTOMLEFT", (6 + 36), 0)
	...
	Reagent:SetHeight(((36 + 6) * (NumRows + 1) + 40) - 6)
...
function Stuffing:SkinWarbandContainer()
	...
	warbandFrame:SetWidth(((36 + 6) * 14) + 17)
	warbandFrame:SetHeight(((36 + 6) * (7 + 1) + 70) - 6)
...
function Stuffing:InitBags()
	...
	for i = 1, #filterTable do
		local button = CreateFrame("Button", "BagsFilterButton"..i, editbox)
...
function Stuffing:Layout(isBank)
	...
	f:SetWidth(cols * 36 + (cols - 1) * 6 + 10 * 2)
	f:SetHeight(rows * 36 + (rows - 1) * 6 + off + 10 * 2)
...
function Stuffing:ADDON_LOADED(addon)
	if addon ~= "TKUI" then return nil end
