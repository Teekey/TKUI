...
function stArch:OnLoad(self)
	...
	if TKUISettingsPerChar.Archaeology == false then self:Hide() end
	-- Title Bar
	...
	stArch["close"]:SetScript("OnMouseUp", function()
		self:Hide()
	...
	for i = 1, numRaces do
		...
		progressBars[i]["bar"]:SetScript("OnEnter", function(self)
			...
			if GetNumArtifactsByRace(i) > 0 then
				...
				ARCHAEOLOGY_RUNE_STONES .. ": " .. numFragmentsCollected .. "/" .. numFragmentsRequired,
					"Keystones: " .. keystoneCount, 1, 1, 1, 1, 1, 1)
				GameTooltip:AddLine(" ")
...
function stArch:UpdateFrameHeight(self)
	...
	if stArch["archSkill"]["frame"]:IsShown() then
		self:SetHeight(stArch["title"]:GetHeight() + stArch["progressBars"]["frame"]:GetHeight() +
		stArch["archSkill"]["frame"]:GetHeight() + 7)
	else
...
function stArch:updateArtifactBar(index)
	...
	if GetNumArtifactsByRace(index) ~= 0 then
		...
		if artifact["numKeysockets"] and artifact["numKeysockets"] > 0 then
			bar["solve"]["text"]:SetText(SOLVE .. " [" .. artifact["numKeystones"] .. "/" ..
			artifact["numKeysockets"] .. "]")
		else
		...
		if GetNumArtifactsByRace(index) > 0 then
			if stArch["artifactInfo"][index]["canSolve"] then
				...
				if (stArch["archSkill"]["rank"] + 5) > stArch["archSkill"]["maxRank"] and stArch["archSkill"]["rank"] ~= stArch["archSkill"]["maxRank"] and not IsShiftKeyDown() then
					print(
					"[|cffe76a6ast|rArch] You should go train before finishing this cast or you won't get all your skill points! (Hold shift if you don't care and still want to complete your artifact now)")
				end
			end
		end
	end)
...
SlashCmdList.STARCHAEOLOGIST = function()
	...
	if stArchFrame:IsShown() then TKUISettingsPerChar.Archaeology = true else TKUISettingsPerChar.Archaeology = false end
end
...
stArchFrame:HookScript("OnEvent", function(self)
	...
	if not select(3, GetProfessions()) then
		stArchFrame:Hide()
		TKUISettingsPerChar.Archaeology = false
	end
	if TKUISettingsPerChar.Archaeology == true and not self:IsShown() then self:Show() end
end)
...
b:SetScript("OnClick", function()
	if _G["stArchaeologyFrame"]:IsShown() then
		...
		TKUISettingsPerChar.Archaeology = true
	end
end)
