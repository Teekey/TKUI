...
local function FilterIcons()
	...
	for index, value in pairs(buttonList) do
		...
		button:SetPoint("BOTTOMRIGHT", ProfessionsFrame.CraftingPage.RecipeList.FilterButton, "TOPRIGHT", -(index - 1) *
		27, 10)
...
local function UpdateTab(object, name, texture, hat)
	...
	local tab = tabs[object][index] or
	CreateFrame("CheckButton", "ProTabs" .. tabs[object].index, object,
...
local function HandleProfession(object, professionID, hat)
	if professionID then
		local _, _, currentSkill, _, numAbilities, offset, skillID, _, _, _, skillLineName = GetProfessionInfo(
		professionID)
