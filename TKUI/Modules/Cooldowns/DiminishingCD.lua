...
function DisplayDrActives(self, test)
	...
	for _, _ in pairs(self.actives) do
		...
		if not aura then
			...
			aura.count:SetFont(C.font.unitframes_font, C.font.unitframes_font_size * 2, C.font.unitframes_font_style)
...
for spell in pairs(T.DiminishingSpells) do
	...
	if not name then
		print("|cffff0000TKUI: DiminishingCD spell ID [" .. tostring(spell) .. "] no longer exists!|r")
	end
end
