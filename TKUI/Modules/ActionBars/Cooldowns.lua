...
local function IsActionBarCooldown(cooldown)
	local parent = cooldown:GetParent()
	if parent then
		local name = parent:GetName()
		if name then
			return name:match("ActionButton") or name:match("MultiBarBottomLeftButton") or
				name:match("MultiBarBottomRightButton") or name:match("MultiBarRightButton") or
				name:match("MultiBarLeftButton")
		end
	end
	return false
end
...
local function Timer_OnSizeChanged(self, width)
	...
	if fontScale < 0.5 then
		...
		self.text:SetFont(C.font.cooldown_timers_font, C.font.cooldown_timers_font_size,
			C.font.cooldown_timers_font_style)
		self.text:SetShadowOffset(C.font.cooldown_timers_font_shadow and 1 or 0,
...
hooksecurefunc(Cooldown_MT, "SetCooldown", function(cooldown, start, duration, modRate)
	...
	if not IsActionBarCooldown(cooldown) then return end
