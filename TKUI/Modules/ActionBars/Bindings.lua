	...
	if not bind.loaded then
		...
		function bind:Listener(key)
			...
			if key == "ESCAPE" or key == "RightButton" then
				...
				print("|cFFFF5C00TK|cFFFFFFFFUI: " ..
					L_BIND_CLEARED .. "|r" .. " |cFFFF5C00" .. self.button.name .. "|r|cffffffff.|r")
				self:Update(self.button, self.spellmacro)
				if self.spellmacro ~= "MACRO" then GameTooltip:Hide() end
				return
			end
			...
			print("|cFFFF5C00TK|cFFFFFFFFUI:|r |cFFFF5C00" ..
				alt .. ctrl .. shift .. key .. "|r |cFFFFFFFFbound to|r |cFFFF5C00" .. self.button.name .. "|r.")
			self:Update(self.button, self.spellmacro)
			if self.spellmacro ~= "MACRO" then GameTooltip:Hide() end
		end
		...
		function bind:Deactivate(save)
			...
			if save then
				SaveBindings(which)
				print("|cFFFF5C00TK|cFFFFFFFFUI: " .. L_BIND_SAVED .. "|r")
			else
				LoadBindings(which)
				print("|cFFFF5C00TK|cFFFFFFFFUI: " .. L_BIND_DISCARD .. "|r")
			end
			self.enabled = false
			self:HideFrame()
